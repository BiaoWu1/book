|**变量**|**容器操作**|**函数**|**面向对象**|**模板与泛型编程**|**内存管理**|**重载运算与类型转换**|
|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
|[变量](#ch1)<br>|[容器操作](#ch4)<br>||[面向对象](#ch6)<br>|[模板与泛型编程](#ch2)<br>|[内存管理](#ch3)<br>|[重载运算与类型转换](#ch5)<br>|
<br>
<br>
...
* [面向对象](#面向对象)
    - [1.定义基类和派生类](#1定义基类和派生类)
        + [1.1定义基类](#11定义基类)
        + [1.2定义派生类](#12定义派生类)
        + [1.3类型转化与继承](#13类型转化与继承)
    - [2.虚函数](#2虚函数)
    - [3.抽象基类](#3抽象基类)
    - [4.访问控制与继承](#4访问控制与继承)
    - [5.继承中的类作用域](#5继承中的类作用域)
<h2 id="ch2"></h2>
...
* [重载运算与类型转换](#重载运算与类型转换)
    ...
    - [3.重载递增递减运算符](#3重载递增递减运算符)
    - [4.重载函数调用运算符](#4重载函数调用运算符)
    - [5.类型转换运算符](#5类型转换运算符)
...
* **溢出**
    ...
    - **赋给signed超过范围的值**：结果未定义，可能继续工作、崩溃、生成垃圾数据<br>
`singed和unsigned，在64或者32位计算机里，int为32位，singed范围为(-2^31 ~ 2^31 - 1),有一位作为符号位.<br>
                                                unsinged范围为(0 ~ 2^32 - 1)，没有符号位<br>
由此可以看出，当确定变量是正数时，unsigned可以扩大其范围`
...
`extern 类型 变量名字 = 值;`（如果在函数内则会报错）  //这是个定义，而且必须定义在函数外部
...
[具体实施](https://blog.csdn.net/wyq2011302646/article/details/38561023)
### 2）const的引用（常量引用）
...
> 头文件中不应该包含using声明，是因为头文件会拷贝到其他文件中去，可能不经意的一些名字会造成史料未及的名字冲突

* **头文件中使用string、vector、cout、cin、list、deque、forward_list**
...
`迭代器的操作类似的是指针操作，因此迭代器的访问时是用->来进行访问的,而pair对象不是指针，通过.运算符进行操作<br>
map<string, size_t> map1;<br>
auto p = map1.insert(make_pair("nihao", 3);<br>
if(p.second)<br>
{<br>
    cout << p.first->second;<br>
}<br>
`
 ...
 * 添加元素
 ...
 >有序容器map，set mutlimap， multiset添加相同元素时对容器无任何影响
 ...
 * 原理
    * 无序关联容器使用一个hash函数和关键字类型的 == 运算符，而不是使用比较运算符
    * 无序容器在存储上为一组桶，使用一个hash函数将元素映射到桶
    * 具有相同hash值的元素在一个桶里，如果容器允许重复关键字，那么所有具有关键字的元素均在同一个桶子里
    
 > 无序容器对关键字的要求：关键字可以是内置类型，但是不能直接定义自定义类型，若要使用自定义类型<br>
   则需要使用自定义hash函数和 == 运算比较
...
* 派生类可以向基类进行转化，是因为每个派生类中都包含一个基类部分，但是只针对对指针或者引用,可以将基类的指针或者引用绑定到派生类对象上有一层极为重要   的含义：**当使用基类的指针（或者）引用时，实际上我们并不清楚改引用（或者指针）所绑定的对象的真实类型**
* 基类向派生类不存在隐式转化
* 派生类向基类的类型转化可能由于访问权限而变得不可访问
    +  
...
* 引用或者指针的静态类型与动态类型不同这一事实正是C++语言支持多态性得根本所在
    - 静态类型：一个对象（变量）的静态类型就是其声明类型，如表达式int a中的int就是对象a的声明类型，即静态类型；
    - 动态类型：`而一个对象（变量）的动态类型就是指程序执行过程中对象（指针或引用）实际所指对象的类型，如Base* pB = new Drived;`<br>
               `其中class Drived继承于class Base，则指针对象pB的静态类型就是Base（声明类型），动态类型就是Drived（实际所指对象的类型）`
               
* 派生类中的虚函数要求：
    + 继承基类而来的虚函数，它的**形参类型**必须与被它覆盖的基类函数完全一致：如果不一致，基类指针或者引用将无法调用派生类的虚函数了.
    + 同样派生类中虚函数的返回类型也必须与基类函数匹配
        - 该规则的一个例外：当返回值是类本身的引用或者指针的时候
        > D 由 B派生而来， 基类虚函数可以返回 B\*, 派生类对应的函数可以返回 D\*<br>
        
        **要求：返回类型要求从D到B的类型转化是可访问的**<br>
            - 当D公有的继承B时，**用户代码**才能使用派生类向基类的转化，如果D继承B的方式时受保护的或者私有的，用户代码不能使用该转化<br>
            - 不论D以什么方式继承B，D的成员函数和友元函数都能使用派生类向基类的转化<br>
            - 如果D继承B的方式是公有或者受保护的，则**D的派生类**的成员和友元都可以使用D向B类型的转换，如果D继承B的方式是私有的，则不能使用<br>
      `假如基类与派生类的虚函数接受的实参不相同，那么我们就无法通过基类的引用或者指针调用派生类的虚函数了`<br>
      
* final和override说明符
    + final运算符
        - 指明类不能被继承：在定义类的时候，类名后面加final
        - 指明函数不能被覆盖：在定义函数的时候，出现在形参列表以及尾置返回类型之后
    + override运算符：标记某个函数，若该函数没有覆盖已存在的虚函数，编译器报错，**出现在形参列表以及尾置返回类型之后**
...
struct D1:B{
    void fi(int) const override;        //正确：fi与接力中的fi匹配
...
//final运算符
struct D2:B {
    void f1(int)const final;           //不允许后续的将其覆盖
};
struct D3 : D2 {
    void f2() override;           //正确，覆盖直接从基类B继承而来的f2
    void f1(int) const;           //错误：D2已经将f2声明为final了，不允许后续的覆盖
};
```
* 回避虚函数机制：使用作用域运算符 ::
...
* 假如调用 p->mem()（或者是引用的obj.mem()）
    + 首先确定p的静态类型
    + 在p的静态类型里面查找mem，如果找不到，依次在直接基类中不断查找直到继承的顶端
    + 一旦找到mem，进行常规的类型检查
    + 假设调用合法，判断是否是虚函数
        - 如果mem是虚函数，而且我们是通过指针或者引用进行调用的，那么产生的代码在运行的时确定到底运行哪个版本
        - 如果mem不是虚函数或者我们时通过对象进行调用的，编译器产生一个常规的函数调用<br>
...
* 对于一个运算符函数而言，它或者是类的成员，或者至少含有一个类类型的参数

> int operator+ (int, int);     //错误，由于没有类类型的参数

* 某些运算符不应该被重载：`逗号运算符、取地址、逻辑与、逻辑或运算符` 

> 由于重载的运算符本质上时一次函数调用，前面的运算符有其原有的求值顺序，重载的运算符不能保证原有运算符的求值顺序<br>
* 使用与内置类型一致的含义
    - 重载运算符的返回类型通常情况下与其内置版本的返回类型兼容
        + 逻辑运算符和关系运算符返回bool类型
        + 算数运算符返回一个类类型的值
        + 赋值运算符和复合赋值运算符(+=)则返回左侧运算对象的引用
    - 如果类的某个操作检查相等性，也即定义了 operator==，那么通常来说需要定义operator!=
    - 如果一个类定义了operator<操作，则一般含有关系的其他操作   
...
* 选择作为成员函数或者非成员函数
    ...
    ```c++
    string s = "a";
    string s1 = s + "b";              //若string的重载运算符+是成员的，那么表示 s.operator("a");
    string s1 = "b" + s;               //若string的重载运算符是成员的，那么表示"a".operator(s),将会引发错误
    ```
    - 非对称性运算符: 必须是成员函数
        + 赋值（=）
        + 下标（[])
...
**改变的是所操作对象的状态，最好设置为成员函数** <br>
        ...
        **参数int必不可少，编译器只有通过参数才能知道应该调用后置版本的++**
...
* 可调用对象和function
    ...
    **function和函数重载出现二义性问题解决方案**<br>
    > int add(int i, int j) { return i + j; }<br>
      Sale_data add(const Sale_data&, const Sale_data&);<br>
