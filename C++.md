...
* 派生类中的虚函数要求：
    ...
    + 同样派生类中虚函数的返回类型也必须与基类函数匹配
        ...
        **要求：返回类型要求从D到B的类型转化是可访问的**<br>
            - 当D公有的继承B时，**用户代码**才能使用派生类向基类的转化，如果D继承B的方式时受保护的或者私有的，用户代码不能使用该转化
            - 不论D以什么方式继承B，D的成员函数和友元函数都能使用派生类向基类的转化
            - 如果D继承B的方式是私有或者受保护的，则**D的派生类**的成员和友元都可以使用D向B类型的转转换，如果D继承B的方式是私有的，则不能使用
